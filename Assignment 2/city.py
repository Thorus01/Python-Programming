...
class City():
    ...
    def __init__(self, name: str, coordinates: Tuple[float, float], city_type: str,\
        ...
        #TODO
        City.id_to_cities.update({self.city_id: self})  #add to the dictionary so this instance can be found by its ID

        if self.name in City.name_to_cities:
            City.name_to_cities[self.name].append(self)
        else:
            City.name_to_cities.update({self.name: [self]})   #see above comment but for name
    ...
    def distance(self, other_city: City) -> int:
        ...
        return math.ceil(geopy.distance.great_circle(self.coordinates, other_city.coordinates).km) #rounding up distance between two cities using great circle method in km
    ...
    def __str__(self) -> str:
        ...
        """
        return f"{self.name} ({self.city_id})"  #returns the city's name and ID
...
def get_city_by_id(city_id: int) -> City | None:
    ...
    """
    if city_id in City.id_to_cities: #if the city ID is found in the id_to_cities dictionary, return it
        return City.id_to_cities[city_id] 
    else:
        return None #returns nothing if city ID not known
...
def get_cities_by_name(city_name: str) -> list[City]:
    ...
    """
    if city_name in City.name_to_cities: #checks if the name of city in name_to_cities dict, if it is, return list of cities known by that name
        return City.name_to_cities[city_name]
    else:
        return [] #else return empty list if no city known
